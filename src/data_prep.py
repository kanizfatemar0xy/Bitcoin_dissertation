...
def calculate_volatility(df):
    """Calculates Bitcoin daily returns and rolling volatility"""
    ...
    # Clean Close column if stored as string (remove commas and dollar signs)
...
def get_sentiment(text):
    """Extracts sentiment polarity score using TextBlob"""
    try:
...
def process_text_dataframe(df, date_col_hint, text_col_hint):
    """Processes text dataset with smart column detection"""
    ...
    # Automatically detect text/title column
    for col in [text_col_hint.lower(), 'title', 'text', 'headline', 'content', 'tweet']:
        if col in df.columns:
            actual_text_col = col
            break
            
    # Raise error if required columns are not found
    ...
    # Convert text column to string to avoid float-related errors
...
def merge_datasets():
    ...
    print("2. Processing News Data (Combining multiple files)...")
    ...
    # Merge price data with news sentiment
